class View:
    ...
    def __eq__(self, other):
        ...
        if not compatible(self, other): # cantidad y tipos de los argumentos
            return False
        ls = getEquivalence(self.head.arguments) # reescribir argumentos de self de forma canonica: X0,X1,.., Xn

        oc = getPredicateListC(other.head.arguments, other.body) # reescribir cuerpo de other para que use forma canonica

        

        return len(self.head.arguments) == len(other.head.arguments) 
           and len(self.body) == len(other.body) and self.constant == other.constant
    """
class Predicate:
