    ...
    for x in query.argset():
        for (i, v) in enumerate(views, 1):
            for y, yp in combinations(v.varset(), r=2):
                if v.is_existential(y) and v.is_existential(yp) and ('t', x.name, y.name) in appearsV[i] and ('t', x.name, yp.name) in appearsV[i]:
#                    global appearsV
...
def add_clauses_D4(query, views, ontology, t):
    ...
    while updated:
        ...
        for (i, v) in enumerate(views, 1):
#            logging.debug("considering v: "+str(i))
            for A in query.constset():
#                logging.debug("considering A: "+str(A))
                for x in query.varset():
#                    logging.debug("considering x: "+str(x))
                    for y in v.argset():
#                        logging.debug("considering y: "+str(y))
                        for z in v.argset():
#                            logging.debug("considering z: "+str(z))
                            if y == z:
                                continue
